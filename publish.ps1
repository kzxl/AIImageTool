...
dotnet publish $hostProj -c Release -r win-x64 -p:SelfContained=false -p:PublishSingleFile=true -o "$slnDir\Publish\Lite"

foreach ($plugin in $pluginsDir) {
    if (Test-Path $plugin) {
        $pluginName = (Get-Item $plugin).BaseName
        Write-Host "Publishing $pluginName Lite..."
        dotnet publish $plugin -c Release -r win-x64 -p:SelfContained=false -o "$slnDir\Publish\Lite\Plugins\$pluginName"
    }
}

# Full Build
Write-Host "Publishing Host Full..."
dotnet publish $hostProj -c Release -r win-x64 -p:SelfContained=true -p:PublishSingleFile=true -o "$slnDir\Publish\Full"

foreach ($plugin in $pluginsDir) {
    if (Test-Path $plugin) {
        $pluginName = (Get-Item $plugin).BaseName
        Write-Host "Publishing $pluginName Full..."
        dotnet publish $plugin -c Release -r win-x64 -p:SelfContained=false -o "$slnDir\Publish\Full\Plugins\$pluginName"
    }
}
