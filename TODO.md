...
- [~] **11.9** Theme: `ThemeManager` + `LightTheme.xaml` + nút đổi Sáng/Tối + lưu setting (áp lúc khởi động).
      **Đã migrate phần lớn panel sang DynamicResource** (thêm key SuccessBrush/DangerBrush/SelectionBrush;
      Filmstrip/ToolsWindow/HistoryPanel/StylePanel/ExportPanel/CollectionsPanel/BatchQueuePanel/
      WorkspaceBrowser/InfoPanel/ImportDialog + DevelopPanel group/slider labels qua SetResourceReference).
      Còn lại: CenterPreview chrome + vài literal rải rác trong DevelopPanel partials (masks/healing) — không
      ảnh hưởng nhiều khi đổi theme. Data-viz (histogram/curve/wheel/overlay) giữ literal cố ý.
